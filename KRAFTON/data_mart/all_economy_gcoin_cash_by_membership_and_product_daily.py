...
# Table: 
#   1. all_economy_gcoin_cash_by_membership_and_product_daily
#   TODO:
#   2. all_economy_gcoin_cash_by_membership_and_product_weekly
#   3. all_economy_gcoin_cash_by_membership_and_product_monthly
...
# device: ["pc", "console"]
# period: ["daily", "weekly", "monthly"] -> 추후 weekly, monthly 추가
def gcoin_cash_by_period(device, period, target_date):
    ...
    if period == "daily":
        ...
        if start_date < F2P_START_DATE:
            start_date = F2P_START_DATE
    elif period == "monthly":
        start_date = (datetime.strptime(target_date, '%Y-%m-%d').replace(day=1)).strftime("%Y-%m-%d")
    else:
        start_date = target_date
        
    user = load_data_mart(device, target_date, target_date, table_name="user_master").where((col("lastlogindate") >= start_date) & (col("server_type") == "LIVE") & (~col("platform").like("%NULL%")))
    ...
    user = user.join(membership, "account_id", "left")
    ...
    groupby_product_list = ["sales_id", "product_id", "product_name"]
    groupby_membership_list = ["device", "platform", "pubg_region", "membership"]
    
    gcoin_use = load_data_mart(device, start_date, target_date, table_name="gcoin_use")
    gcoin_use_by_user = gcoin_use.groupBy(["account_id"]+groupby_product_list).agg(sum("qty").alias("unit_sold"), sum("free_use").alias("free_gcoin_used"), sum("paid_use").alias("paid_gcoin_used")).withColumn("ingame_revenue", lit(None)).withColumn("currency", lit("gcoin"))
    
    cash_mtx = load_data_mart(device, start_date, target_date, table_name="cash_mtx")
    cash_mtx_by_user = cash_mtx.groupBy(["account_id"]+groupby_product_list).agg(sum("unit_sold").alias("unit_sold"), sum("ingame_revenue").alias("ingame_revenue")).withColumn("free_gcoin_used", lit(None)).withColumn("paid_gcoin_used", lit(None)).withColumn("currency", lit("cash"))
    
    gcoin_cash_by_user = gcoin_use_by_user.unionByName(cash_mtx_by_user).join(user, "account_id", "left")
    
    gcoin_cash_by_product = gcoin_cash_by_user.groupBy(groupby_membership_list+groupby_product_list+["currency"]).agg(countDistinct("account_id").alias("pu"), sum("unit_sold").alias("unit_sold"), sum("free_gcoin_used").alias("free_gcoin_used"), sum("paid_gcoin_used").alias("paid_gcoin_used"), sum("ingame_revenue").alias("ingame_revenue"))
    gcoin_cash_by_membership = gcoin_cash_by_user.groupBy(groupby_membership_list).agg(countDistinct("account_id").alias("pu"), sum("unit_sold").alias("unit_sold"), sum("free_gcoin_used").alias("free_gcoin_used"), sum("paid_gcoin_used").alias("paid_gcoin_used"), sum("ingame_revenue").alias("ingame_revenue")).withColumn("currency", lit("gcoin+cash")).withColumn("sales_id", lit(0)).withColumn("product_id", lit("total")).withColumn("product_name", lit(0))
    gcoin_cash_by_membership_and_currency = gcoin_cash_by_user.groupBy(groupby_membership_list+["currency"]).agg(countDistinct("account_id").alias("pu"), sum("unit_sold").alias("unit_sold"), sum("free_gcoin_used").alias("free_gcoin_used"), sum("paid_gcoin_used").alias("paid_gcoin_used"), sum("ingame_revenue").alias("ingame_revenue")).withColumn("sales_id", lit(0)).withColumn("product_id", lit("total")).withColumn("product_name", lit(0))
    gcoin_cash_all = gcoin_cash_by_product.unionByName(gcoin_cash_by_membership).unionByName(gcoin_cash_by_membership_and_currency)
    gcoin_cash_all = gcoin_cash_all.withColumn("membership", coalesce(col("membership"), lit("undefined")))
    
    au = user.groupBy(groupby_membership_list).agg(countDistinct("account_id").alias("au")).withColumn("membership", coalesce(col("membership"), lit("undefined")))
    gcoin_cash_final = gcoin_cash_all.join(au, groupby_membership_list, "outer").withColumn("date", lit(target_date))
    
    return gcoin_cash_final

for period in ["daily"]:
    try:
        pc_gcoin_cash = gcoin_cash_by_period("pc", period, target_date)
        console_gcoin_cash = gcoin_cash_by_period("console", period, target_date)
        
        all_gcoin_cash = pc_gcoin_cash.unionByName(console_gcoin_cash).withColumn("reg_datetime", lit(datetime.now()))
        all_gcoin_cash = all_gcoin_cash.select("date", "device", "platform", "pubg_region", "membership", "sales_id", "product_id", "product_name", "currency", "au", "pu", "unit_sold", "free_gcoin_used", "paid_gcoin_used", "ingame_revenue", "reg_datetime")
        
        db_name = "pubg_gi"
        table_name = "all_economy_gcoin_cash_by_membership_and_product_{}".format(period)
        
        delete_for_rebatch("pc", target_date, table_name)
        delete_for_rebatch("console", target_date, table_name)
        mysql.insert_table(all_gcoin_cash, db_name, table_name)
    
    except Exception as e:
        slack.send(CHANNEL, "Error occurred in daily_gb_pdu_seoyoung_all_economy_gcoin_cash_by_membership_and_product.py, \n target date: {} \n".format(target_date) + str(e), "danger")
