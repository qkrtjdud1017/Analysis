...
# Table: 
#   pubg_gi.all_economy_wsus_contraband_coupon_by_membership_daily
...
TABLE_NAME = "all_economy_wsus_contraband_coupon_by_membership_daily"
...
def get_contrabandcoupon_balance(device, target_date):
    ...
    currency = load_schema.lobby(spark, device, "live", "CurrencyIncreased", start_date=target_date, end_date=target_date).withColumnRenamed("AccountId", "account_id")
    balance = currency.where((col("Currency") == "currencydesc.contrabandcoupon")).groupBy("account_id", "Reason", "ReasonDetail").agg(sum("Amount").alias("balance"))

    df = balance.join(user, "account_id", "left")
    contracoupon_by_membership = df.groupBy("device", "platform", "pubg_region", "membership", "Reason", "ReasonDetail").agg(sum("balance").alias("balance")).withColumn("date", lit(target_date)).withColumn("reg_datetime", lit(datetime.now())).select("date", "device", "platform", "pubg_region", "membership", "Reason", "ReasonDetail", "balance", "reg_datetime")
    
    return contracoupon_by_membership

for device in ["pc", "console"]:
    try:
        coupon_balance = get_contrabandcoupon_balance(device, target_date)
        delete_for_rebatch(device, target_date, TABLE_NAME)
        mysql.insert_table(coupon_balance, "pubg_gi", TABLE_NAME)
    
    except Exception as e:
        slack.send(CHANNEL, "Error occurred in daily_gb_pdu_seoyoung_all_economy_wsus_contraband_coupon_by_membership_daily.py, \n target date: {} \n device: {} \n".format(target_date, device) + str(e), "danger")
