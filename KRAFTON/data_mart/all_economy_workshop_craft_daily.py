...
# Table: 
#   pubg_gi.all_economy_workshop_craft_daily
# Duration: 3 minutes
...
def get_craft(device, target_date):
    craft = load_schema.lobby(spark, device, "live", "WorkshopCrafted", target_date, target_date)
    df = craft.groupBy("ItemDescId").agg(count("*").alias("craft_count"), countDistinct("AccountId").alias("user_count")) \
        .withColumnRenamed("ItemDescId", "itemdescid").withColumn("date", lit(target_date)).withColumn("device", lit(device)).withColumn("reg_datetime", lit(datetime.now())) \
        .select("date", "device", "itemdescid", "craft_count", "user_count", "reg_datetime")
    return df

for device in ["pc", "console"]:
    if device == "console" and target_date < CONSOLE_WORKSHOP_START_DATE:
        slack.send(CHANNEL, "target_date earlier than workshop_start_date in daily_gb_pdu_seoyoung_all_economy_workshop_craft_daily.py, \n device: {} \n target date: {} \n".format(device, target_date))
    else:
        try:
            craft_by_item = get_craft(device, target_date)
            table_name = "all_economy_workshop_craft_daily"
            delete_for_rebatch(device, target_date, table_name)
            mysql.insert_table(craft_by_item, "pubg_gi", table_name)
            slack.send(CHANNEL, "{} for {} date: {} Succeeded".format(table_name, device, target_date), 'good')
        except Exception as e:
            slack.send(CHANNEL, "Error occurred in daily_gb_pdu_seoyoung_all_economy_workshop_craft_daily.py, \n device: {} \n target date: {} \n".format(device, target_date) + str(e), "danger")
