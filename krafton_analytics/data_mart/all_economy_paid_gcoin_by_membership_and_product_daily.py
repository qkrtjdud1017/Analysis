...
# Table: 
#   1. all_economy_paid_gcoin_by_membership_and_product_daily
# S3: 
# Duration: 5 minutes
...
def delete_for_rebatch(period, device, target_date, table_name):
    """
...
# device: ["pc", "console"]
# possible period: ["daily", "weekly", "monthly"]
def paid_gcoin_by_period(device, period, target_date):
    ...
    gcoin_use = load_data_mart(device, start_date, target_date, table_name="gcoin_use").where(col("paid_use") > 0)
    gcoin_use_by_user = gcoin_use.groupBy(["account_id"]+groupby_product_list).agg(sum("qty").alias("unit_sold"), sum("free_use").alias("free_gcoin_used"), sum("paid_use").alias("paid_gcoin_used")).withColumn("ingame_revenue", lit(None)).join(user, "account_id", "left")
    gcoin_use_by_product = gcoin_use_by_user.groupBy(groupby_membership_list+groupby_product_list).agg(countDistinct("account_id").alias("pu"), sum("unit_sold").alias("unit_sold"), sum("free_gcoin_used").alias("free_gcoin_used"), sum("paid_gcoin_used").alias("paid_gcoin_used"))
    gcoin_use_by_membership = gcoin_use_by_user.groupBy(groupby_membership_list).agg(countDistinct("account_id").alias("pu"), sum("unit_sold").alias("unit_sold"), sum("free_gcoin_used").alias("free_gcoin_used"), sum("paid_gcoin_used").alias("paid_gcoin_used")).withColumn("sales_id", lit(0)).withColumn("product_id", lit("total")).withColumn("product_name", lit(0)).withColumn("membership", coalesce(col("membership"), lit("undefined")))
    
    gcoin_final = gcoin_use_by_product.unionByName(gcoin_use_by_membership)
    ...
    if period == "daily":
        df = gcoin_final.join(au, groupby_membership_list, "outer").withColumn("date", lit(target_date)).withColumn("sales_id", coalesce(col("sales_id"), lit(0))).withColumn("product_id", coalesce(col("product_id"), lit(0))).na.drop(subset=["device", "platform", "pubg_region", "membership", "sales_id", "product_id"])
    else:
        df = gcoin_final.join(au, groupby_membership_list, "outer").withColumn("start_date", lit(start_date)).withColumn("end_date", lit(target_date)).withColumn("sales_id", coalesce(col("sales_id"), lit(0))).withColumn("product_id", coalesce(col("product_id"), lit(0))).na.drop(subset=["device", "platform", "pubg_region", "membership", "sales_id", "product_id"])
    return df

for period in ["daily"]:
    try:
        pc_gcoin = paid_gcoin_by_period("pc", period, target_date)
        console_gcoin = paid_gcoin_by_period("console", period, target_date)
        
        all_gcoin = pc_gcoin.unionByName(console_gcoin).withColumn("reg_datetime", lit(datetime.now()))
        if period == "daily":
            all_gcoin = all_gcoin.select("date", "device", "platform", "pubg_region", "membership", "sales_id", "product_id", "product_name", "au", "pu", "unit_sold", "free_gcoin_used", "paid_gcoin_used", "reg_datetime")
        else:
            all_gcoin = all_gcoin.select("start_date", "end_date", "device", "platform", "pubg_region", "membership", "sales_id", "product_id", "product_name", "au", "pu", "unit_sold", "free_gcoin_used", "paid_gcoin_used", "reg_datetime")
         
        db_name = "pubg_gi"
        table_name = "all_economy_paid_gcoin_by_membership_and_product_{}".format(period)
        
        delete_for_rebatch(period, "pc", target_date, table_name)
        delete_for_rebatch(period, "console", target_date, table_name)
        mysql.insert_table(all_gcoin, db_name, table_name)
    
    except Exception as e:
        slack.send(CHANNEL, "Error occurred in daily_gb_pdu_seoyoung_all_economy_paid_gcoin_by_membership_and_product.py, \n target date: {} \n period: {} \n".format(target_date, period) + str(e), "danger")
