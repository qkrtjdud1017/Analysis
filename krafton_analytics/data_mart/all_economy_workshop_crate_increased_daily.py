...
# Table: 
#   pubg_gi.all_economy_workshop_crate_increased_daily
# Duration: 5 minutes
...
def get_crate_increased(device, target_date):
    crateIncreased = load_schema.lobby(spark, device, "live", "WorkshopCrateIncreased", target_date, target_date)
    crate_df = crateIncreased.groupBy("ItemDescId", "Reason").agg(sum(col("Amount")).alias("amount"))
    crate_df = crate_df.withColumn("date", lit(target_date)).withColumn("device", lit(device)).withColumn("reg_datetime", lit(datetime.now())) \
...
for device in ["pc", "console"]:
    if device == "console" and target_date < CONSOLE_WORKSHOP_START_DATE:
        ...
        try:
            crate_increased = get_crate_increased(device, target_date)
            table_name = "all_economy_workshop_crate_increased_daily"
            delete_for_rebatch(device, target_date, table_name)
            mysql.insert_table(crate_increased, "pubg_gi", table_name)
            slack.send(CHANNEL, "{} for {} date: {} Succeeded".format(table_name, device, target_date), 'good')
        except Exception as e:
            slack.send(CHANNEL, "Error occurred in daily_gb_pdu_seoyoung_all_economy_workshop_crate_increased_daily.py, \n device: {} \n target date: {} \n".format(device, target_date) + str(e), "danger")
